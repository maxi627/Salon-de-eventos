...
from app.services import AdministradorService, ResponseBuilder
...
def delete(id):
    ...
    try:
        ...
        return response_schema.dump(response_builder.build()), 500
@Administrador.route('/administrador/<int:id>/manage', methods=['POST'])
@limiter.limit("5 per minute")
def manage(id):
    response_builder = ResponseBuilder()
    try:
        ...
        # Llamar a la función de gestión de administrador
        updated_administrador = service.manage_administrador(id, cantidad)

        # Devolver el administrador actualizado
        response_builder.add_message("Administrador updated").add_status_code(200).add_data(updated_administrador)
        return response_schema.dump(response_builder.build()), 200
        ...
        response_builder.add_message("Error managing administrador").add_status_code(500).add_data(str(e))
        return response_schema.dump(response_builder.build()), 500
